ganet.combine <- function(BIOGRID=1, CORUM=1, DIP=1,HPRD=1, INNATEDB=1, INTACT=1,MINT=1,MPI=1, NAT=0,REACT=0)
{
  ...
  if(CORUM ==1) {data(corum); CombinedDatabases <- rbind(CombinedDatabases, corum[,1:2])}
  if(DIP ==1) {data(dip); CombinedDatabases <- rbind(CombinedDatabases, dip[,1:2])}
  if(HPRD ==1)  {data(hprd); CombinedDatabases <- rbind(CombinedDatabases, hprd[,1:2]) }
  if(INNATEDB ==1)  {data(innatedb); CombinedDatabases <- rbind(CombinedDatabases, innatedb[,1:2]) }
  ...
  if(MPI ==1)  {data(mpi);CombinedDatabases <- rbind(CombinedDatabases, mpi[,1:2])}
  #pathways are the followings
  if(NAT ==1)  {data(nat);CombinedDatabases <- rbind(CombinedDatabases, nat[,1:2]) }
  if(REACT ==1) {data(reactome); CombinedDatabases <- rbind(CombinedDatabases, reactome[,1:2])}
